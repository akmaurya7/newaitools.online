...
export const BlogListing: React.FC = () => {
  ...
  return <main className="min-h-[70vh] bg-[#f8f7f4]">
    ...
    <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 md:py-16 lg:px-8">
      ...
      {BLOG_POSTS.length ? <div className="mt-12"><div className="mb-6 flex items-end justify-between gap-4"><div><p className="mb-2 text-xs font-bold uppercase tracking-[0.18em] text-accent">Latest research</p><h2 className="font-serif text-3xl text-ink md:text-4xl">Latest articles</h2></div><span className="text-sm text-ink/45">{BLOG_POSTS.length} published</span></div><div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">{BLOG_POSTS.slice().sort((a, b) => b.publishDate.localeCompare(a.publishDate)).map(post => <article key={post.id} className="group flex flex-col rounded-2xl border border-ink/[0.08] bg-white p-6 transition hover:-translate-y-0.5 hover:border-accent/25 hover:shadow-lg">{post.ogImage && <img src={post.ogImage} alt={post.ogImageAlt || post.title} width="800" height="450" loading="lazy" decoding="async" className="-mx-6 -mt-6 mb-5 aspect-[16/9] w-[calc(100%+3rem)] rounded-t-2xl bg-ink/5 object-cover" />}<div className="mb-5 flex items-center justify-between gap-3"><span className="rounded-full border border-accent/10 bg-orange-50 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-accent">{post.category}</span><span className="text-xs text-ink/40">{post.readTime} min read</span></div><h3 className="font-serif text-xl leading-snug text-ink transition group-hover:text-accent"><a href={`/blog/${post.slug}`}>{post.title}</a></h3><p className="mt-3 flex-grow text-sm leading-6 text-ink/55">{post.excerpt}</p><div className="mt-5 flex items-end justify-between gap-3 border-t border-ink/[0.07] pt-4"><time dateTime={post.publishDate} className="text-xs text-ink/40">{new Date(post.publishDate).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</time><a href={`/blog/${post.slug}`} className="inline-flex items-center gap-1.5 text-sm font-bold text-accent">Read article <ArrowRight size={15} className="transition group-hover:translate-x-1" /></a></div></article>)}</div></div> : <div className="mt-10 rounded-2xl border border-dashed border-ink/15 bg-white px-6 py-12 text-center sm:py-16"><span className="mx-auto flex h-12 w-12 items-center justify-center rounded-xl bg-[#f6eee8] text-accent"><CheckCircle2 size={21} /></span><h2 className="mt-5 font-serif text-2xl text-ink">The first research-led articles are in progress.</h2><p className="mx-auto mt-2 max-w-xl text-sm leading-6 text-ink/55">We’re building this library around useful tool reviews, clear comparisons, and workflows grounded in current sources.</p><a href="/tools" className="mt-6 inline-flex items-center gap-2 rounded-xl bg-accent px-5 py-3 text-sm font-bold text-white transition hover:bg-ink">Browse the tool directory <ArrowRight size={15} /></a></div>}
    </section>
