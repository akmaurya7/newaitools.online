...
export const BlogSection: React.FC = () => <section id="blog" className="border-y border-ink/[0.07] bg-white py-14 md:py-16">
  <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
    ...
    <div className="grid grid-cols-1 gap-4 md:grid-cols-3">{BLOG_POSTS.length ? BLOG_POSTS.slice().sort((a, b) => b.publishDate.localeCompare(a.publishDate)).slice(0, 3).map(post => <a key={post.id} href={`/blog/${post.slug}`} className="group rounded-2xl border border-ink/[0.08] bg-[#f8f7f4] p-5 transition hover:-translate-y-0.5 hover:border-accent/25 hover:bg-white hover:shadow-lg"><span className="text-[10px] font-bold uppercase tracking-wider text-accent">{post.category} · {post.readTime} min read</span><h3 className="mt-3 font-serif text-xl leading-snug text-ink transition group-hover:text-accent">{post.title}</h3><p className="mt-2 line-clamp-3 text-sm leading-6 text-ink/55">{post.excerpt}</p><span className="mt-4 inline-flex items-center gap-1.5 text-sm font-semibold text-accent">Read article <ArrowRight size={14} /></span></a>) : topics.map(({ icon: Icon, title, text }) => <a key={title} href="/blog" className="group rounded-2xl border border-ink/[0.08] bg-[#f8f7f4] p-5 transition hover:-translate-y-0.5 hover:border-accent/25 hover:bg-white hover:shadow-lg"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#f6eee8] text-accent"><Icon size={19} /></span><h3 className="mt-4 font-serif text-xl text-ink transition group-hover:text-accent">{title}</h3><p className="mt-2 text-sm leading-6 text-ink/55">{text}</p></a>)}</div>
  </div>
</section>;
